...
from pyflow.lib.PADS.Util import arbitrary_item

import sys
import operator
import unittest

# sets.Set and sets.ImmutableSet are deprecated in Python 3, use built-in set and frozenset instead
from pyflow.lib.PADS.PartitionRefinement import PartitionRefinement
from pyflow.lib.PADS.Sequence import Sequence
    ...
    def pprint(self,output=sys.stdout):
        ...
        for state in self.states():
            ...
            if not adjectives:
                ...
                print(state, "(" + ", ".join(adjectives) + ")", file=output)
            for c in self.alphabet:
                for neighbor in self.transition(state,c):
                    print("  --[" + str(c) + "]-->", neighbor, file=output)
