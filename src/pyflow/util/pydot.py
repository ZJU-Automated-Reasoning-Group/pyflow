...
class Common(object):
    ...
    def _get_default_attribute(self, attr):
    ...
    def set(self, name, value):
        ...
        if name in self.obj_dict["attributes"]:
...
class InvocationException(Exception):
    """To indicate that a problem occurred while running any of the
    ...
    def __eq__(self, edge):
        ...
        if self.get_parent_graph().get_top_graph_type() == "graph":
            ...
            if (
                ...
                and self.get_destination() == edge.get_source()
            ):
                return True
    ...
    def get_edge(self, src, dst):
        ...
        if (src, dst) in self.obj_dict["edges"] or (
            ...
            edges_obj_dict = self.obj_dict["edges"].get(
                (src, dst), self.obj_dict["edges"].get((dst, src), None)
            )
...
class Dot(Graph):
    ...
    def create(self, prog=None, format="ps"):
        ...
        for img in self.shape_files:
            ...
            #
            with open(os.path.join(tmp_dir, os.path.basename(img)), "wb") as f:
                f.write(f_data)
