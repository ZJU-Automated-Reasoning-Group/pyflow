...
class Collapser(TypeDispatcher):
    ...
    def process(self, node):
        ...
        if (
            isinstance(node, ast.Statement)
            and not isinstance(node, ast.Assign)
            and not isinstance(node, ast.Suite)
        ):
            self.resetStack()
    ...
    def visitBuildSlice(self, node):
        ...
        if node.stop:
            self.process(node.stop)
        if node.start:
            self.process(node.start)

        if node.start:
            self.markPossible(node.start)
        if node.stop:
            self.markPossible(node.stop)
        if node.step:
            self.markPossible(node.step)
