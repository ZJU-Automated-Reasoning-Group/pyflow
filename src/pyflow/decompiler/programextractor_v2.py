...
import pyflow.language.python.program as program
...
from pyflow.application.errors import TemporaryLimitation, InternalError
...
def type_fields(t):
	...
	for cls in inspect.getmro(t):
		fields.update(cls.__dict__.keys())
	return fields
...
##			assert self.complete[obj], obj
	...
	def handleContainer(self, obj):
		if isinstance(obj.pyobj, (dict, xtypes.DictProxyType)):
			...
			keys = set(obj.pyobj.keys())
			keys.update(lut.keys())
			...
			for i, v in zip(list(range(len(obj.pyobj))), obj.pyobj):
	...
	def decompileFunction(self, func, trace=False, ssa=True, descriptive=False):
		...
		try:
		...
		except errors.UnsupportedOpcodeError as e:
			...
			self.failiures += 1
		except TemporaryLimitation as e:
...
from pyflow.stubs import makeStubs
