...
import pyflow.decompiler.errors

from pyflow.util.python import moduleForGlobalDict

from pyflow.decompiler.flowblocks import *

import collections

from pyflow.decompiler.destacker import destack
from . structuralanalyzer import StructuralAnalyzer
from . import ssitransform

from pyflow.analysis.cfg import ssatransform

from pyflow.decompiler.flowblockdump import FlowBlockDump
from pyflow.language.python.simplecodegen import SimpleCodeGen

import pyflow.optimization.simplify

from pyflow.language.python.annotations import codeOrigin
...
def decompile(compiler, func, trace=False, ssa=True, descriptive=False):
	...
	pyflow.optimization.simplify.evaluateCode(compiler, None, code)
...
class BlockBuilder(object):
	...
	def eliminateNullBlocks(self):
		...
		for i, outs in self.instOut.items():
			for o in outs:
		...
		# Rename the exits.
		newExit = {}
		for r, (a, b) in self.regionExit.items():
